...
# /// Dane i ich opis mozna znalezc na stronie http://www.ats.ucla.edu/stat/stata/dae/discrim.htm
# 
# /// Przyk³ad 1.
# 
...
dataset <- read.dta('./data/discrim.dta')
...
summary(dataset[,c('outdoor','social','conservative')])
...
dataset %>% group_by(job) %>% 
  summarize(mean_outdoor=mean(outdoor),
            mean_social=mean(social),
            mean_cons=mean(conservative),
            sd_outdoor=sd(outdoor),
            sd_social=sd(social),
            sd_cons=sd(conservative))

# /// Tablica korelacji. Opcja sig wyswietla wartosc p dla hipotezy o braku korelacji
dataset_to_plot <- dataset[,c('outdoor','social','conservative')] %>% as.matrix()
str(dataset_to_plot)
?cor.plot
cor.plot(dataset_to_plot)
c('outdoor','social','conservative') %>% combn(2) %>% 
  adply(2,.fun=function(x){
    cor.plot(dataset[,x[1]],dataset[,x[2]])
